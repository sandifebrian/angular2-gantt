...
export class GanttService {
    ...
    public gridWidth: number = 426; // 560
    ...
    private calculateBarLeft(start: Date, scale: any[], hours?: boolean): number {
        ...
        if (start != null) {
            if (typeof start === "string") {
                start = new Date(start); // changed by sandi.febrian@gmail.com  from "start = new Date();"
            }

            /**
             * START getting start date ( by sandi.febrian@gmail.com )
             */
            let month = start.getUTCMonth() + 1; // months from 1-12
            let day = start.getUTCDate();
            let year = start.getUTCFullYear();
            const barDate = year + '/' + month + '/' + day;
            /**
             * END start date
             */
            ...
            for (var i = 0; i < scale.length; i++) {
                /**
                 * START getting scale date add by sandi.febrian@gmail.com
                 */
                month = scale[i].getUTCMonth() + 1; // months from 1-12
                day = scale[i].getUTCDate();
                year = scale[i].getUTCFullYear();
                const scaleDate = year + '/' + month + '/' + day;
                /**
                 * END scale date
                */

                if (barDate === scaleDate) {
                    if (hours) {
    ...
    public calculateActivityContainerDimensions(): any {
        var scrollWidth: number = 10; // 18;
        console.log('gantService 450 :', window.document.getElementsByClassName('gantt_container')[0]);
        this.windowInnerWidth = window.document.getElementsByClassName('gantt_container')[0].offsetWidth;
        // this.windowInnerWidth = window.innerWidth;
